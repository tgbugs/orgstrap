# -*- org-adapt-indentation: nil; org-edit-src-content-indentation: 0; orgstrap-cypher: sha256; orgstrap-block-checksum: fddd77698e7701d0bde10ac5a9d04186aea14aee3ff5635830d6500fbe4a1587; orgstrap-norm-func-name: orgstrap-norm-func--prp-1\.1; -*-
...
(defvar orgstrap-clone--body-params nil
  "Current source block body and parameters for clone operations.")
(defvar orgstrap-clone--source-buffer nil
  "Source buffer whose orgstrap block serves as the source for clone operations.")
(defun orgstrap-clone-mark-source ()
  "Mark the current buffer as the source for `orgstrap-clone'."
  (interactive)
  (if (orgstrap--used-in-current-buffer-p)
      (setq orgstrap-clone--source-buffer (current-buffer))
    (message "orgstrap is not used in the current buffer!")))

(defun orgstrap-clone-mark-block ()
  "Mark an arbitrary source block to clone to an orgstrap block."
  (interactive)
  (error "Not implemented. TODO"))

(defun orgstrap-update-src-block-header (name new-params)
...
NEW-PARAMS."
  (let ((defaults (append org-babel-default-header-args
    ...
    (orgstrap--with-block name
      ...
      (dolist (pair new-params)
        (cl-destructuring-bind (key . value)
            pair
          (when (and value
                     (not (memq key '(:result-params :result-type)))
                     (not (member pair defaults)))
            (let ((header-arg (symbol-name key)))
              (org-babel-insert-header-arg header-arg value))))))))

(defun orgstrap-clone (&optional universal-argument overwrite)
  "Clone orgstrap block via `orgstrap-clone-mark-source' to current buffer.
If UNIVERSAL-ARGUMENT is '(64) aka (C-u C-u) this will OVERWRITE any existing
block.  If you are not calling this interactively all as (orgstrap-clone nil t)
for calirty.  You cannot clone an orgstrap block into its own buffer."
  (interactive "P")
  (let ((overwrite (or overwrite (equal universal-argument '(16))))
        (source-buffer orgstrap-clone--source-buffer)
        (target-buffer (current-buffer)))
    (when (eq source-buffer target-buffer)
      (error "Source and target are the same buffer!"))
    (cl-destructuring-bind (source-body . source-params)
        (with-current-buffer source-buffer
          (orgstrap--with-block orgstrap-orgstrap-block-name
            (ignore body-unexpanded)
            (cons body params)))
      (orgstrap--add-orgstrap-block source-body)
      (orgstrap-update-src-block-header orgstrap-orgstrap-block-name))))
...
:END:
** 1.2.4
- Add ~orgstrap-clone~ and ~orgstrap-clone-mark-source~ commands.

  ~orgstrap-clone~ clones another orgstrap block from a buffer marked
  via ~orgstrap-clone-mark-source~ to be the block for the current
  buffer. Useful in cases where users want to duplicate functionality
  in a new file.

- ~orgstrap--add-orgstrap-block~ add ~block-contents~ argument.

  This simplifies the implementation of clone, and makes it possible
