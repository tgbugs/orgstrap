...
;; Version: 1.5.6                             (ref:orgstrap.el-version)
...
emacs-29 -Q $THIS_FILE
emacs-30 -Q $THIS_FILE
#+end_src
...
emacs-29 -Q orgstrap-minimal.org
emacs-30 -Q orgstrap-minimal.org
#+end_src
*** Matrix
...
(ow-run-command "emacs-27" "-batch" "-f" "batch-byte-compile" "orgstrap.el")
...
- Update ~orgstrap--shebang-body~ for Emacs 30.1 and Org 9.7.11.
  ...
  Workaround for issue where ~org-restart-font-lock~ is not defined
  when defcustom tries to run ~org-link--set-link-display~.
