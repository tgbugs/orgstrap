...
#+end_src
*** +reval+ reload
Remote +eval+ load. Ephemoral code reuse. Somewhere between load
eval, and securl then load. More auditable and stable curl pipe bash.
#+begin_src elisp :lexical yes
;; FIXME TODO dedupe this code
(defun resum (cypher path-or-url)
  "Return checksum under CYPHER for PATH-OR-URL."
  (cl-flet ((do-the-thing
             ()
             (let (enable-local-eval)
               (save-window-excursion
                 (with-current-buffer (find-file path-or-url)
                   (intern (secure-hash cypher (buffer-string))))))))
    (if url-handler-mode
        (do-the-thing)
      (unwind-protect
          (progn
...
(defun make-reval (cypher path-or-url)
  `(reval ,cypher ',(resum cypher path-or-url) ,path-or-url))

(defun reval (cypher checksum path-or-url)
             ...
             (let (enable-local-eval)
               (save-window-excursion
                 (with-current-buffer (find-file path-or-url)
                   ...
                   (let ((buffer-checksum (intern (secure-hash cypher (buffer-string)))))
...
(reval sha256 '6c0e1703016725caef7b0b8c25955cebe6ad72624386c2d45752bbf53763bbf0 "https://github.com/tgbugs/orgstrap/blob/417b87304da27397d3ce1b79039119a9337be305/packages.el")
#+end_src
*** securl
