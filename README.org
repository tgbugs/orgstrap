...
| file-exists-p | checksum-match | multi | signal | buffer |
|---------------+----------------+-------+--------+--------|
| t             | t              | t     | warn   | evaled |
| nil           | t              | t     | warn   | kill   |
| t             | nil            | t     | warn   | failed |
| nil           | nil            | t     | warn   | kill   |
| t             | t              | nil   | error  | evaled |
| nil           | t              | nil   | error  | kill   |
| t             | nil            | nil   | error  | failed |
| nil           | nil            | nil   | error  | kill   |
...
(defun reval-make (cypher path-or-url)
  `(reval ',cypher ',(reval-resum cypher (reval-id->buffer path-or-url) t) ,path-or-url))
...
(defun reval--add-buffer-to-list (buffer buffer-list-name)
  (with-current-buffer buffer ; FIXME do this in both cases but change which list
    ...
    (lexical-let ((buffer-list-name buffer-list-name))
      ;; even with lexical t we still have to use `lexical-let' to get
      ;; `buffer-list-name' to bind as expected
  ...
  (let (done)
    (with-url-handler-mode
      (cl-loop for path-or-url in (cons path-or-url alternates)
               do (let ((buffer (reval-id->buffer path-or-url)))
                    (if (file-exists-p (buffer-file-name buffer))
                        (let ((buffer-checksum (reval-resum cypher buffer)))
                          (if (and buffer-checksum
...
;; Reval-Get-Immutable: ow--update-url
