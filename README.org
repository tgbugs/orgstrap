...
(defcustom orgstrap-do-config nil "Run code that modifies the Emacs config")
...
(defcustom orgstrap-do-packages nil "Install all packages.")

(defcustom orgstrap-do-packages-emacs nil "Install Emacs packages.")
(defcustom orgstrap-do-packages-system nil "Install system packages.")

;(defcustom orgstrap-do-setup nil "Run other setup steps.")  ; FIXME this is closer to emerge --config
(defcustom orgstrap-do-services nil "Run services needed by file.")
;; other parts of the software lifecycle
(defcustom orgstrap-do-package nil "???")
(defcustom orgstrap-do-release nil "???")
    ...
    (save-window-excursion
      (with-current-buffer buffer
        (when review
          ...
          ;; FIXME `yes-or-no-p' breaks all keyboard input :/
...
(defun resum-minimal (cypher buffer)
  ...
  (save-excursion
    ...
    (let ((begin (point)))
      ...
      (let* ((raw (read (buffer-substring-no-properties begin (point))))
             ...
             (checksum (resum (elt raw 1) buffer t)))
...
(defun reval-minimal (cypher checksum path-or-url)
