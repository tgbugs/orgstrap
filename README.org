# -*- org-adapt-indentation: nil; org-edit-src-content-indentation: 0; orgstrap-cypher: sha256; orgstrap-block-checksum: 8b001890aa741d9e9acb2f553b8bbba2d7b87f5bfe78d2764eff3c63e473af84; orgstrap-norm-func-name: orgstrap-norm-func--prp-1\.1; -*-
...
(defvar orgstrap-do-build/make nil ; FIXME clarify naming
  ;; there is tangle -> make and there is build -> tangle or something?
  "")
...
this functionality.")
(define-obsolete-variable-alias 'orgstrap-tangle 'orgstrap-do-tangle)

(defvar orgstrap-do-test nil
...
(define-obsolete-variable-alias 'orgstrap-test 'orgstrap-do-test)
...
;; Version: 1.2.5                             (ref:orgstrap.el-version)
    ...
    (save-window-excursion
      (with-current-buffer buffer
        (when review
          ...
          (unless (yes-or-no-p "Audit of file ok? " nil) ; not using `y-or-n-p' since it is too easy
...
(defun reval-make (cypher path-or-url)
  `(reval ',cypher ',(reval-resum cypher (id->buffer path-or-url) t) ,path-or-url))
...
If an error is encountered try ALTERNATES in order."
  (let (done)
    (cl-loop for path-or-url in (cons path-or-url alternates)
             do (let* ((buffer (id->buffer path-or-url))
                       (buffer-checksum (reval-resum cypher buffer)))
                  (if (eq buffer-checksum checksum)
                      (progn
                        (eval-buffer buffer)
                        (push buffer reval-evaled-buffer-list)
                        (setq done t))
                    (with-current-buffer buffer ; FIXME do this in both cases but change which list
                      ...
                      (add-hook 'kill-buffer-hook
                                ...
                                nil t))
                    (if (with-url-handler-mode (file-exists-p path-or-url))
                        ;; TODO probably need to signal something more specific
...
;; today we learn about & key key insert of digraphgs !? まとせれめはそらてどööて∃∀
(defun reval-minimal (cypher checksum path-or-url &rest _alts)
  ...
  ;; issue on their own
  (let* ((buffer (let ((o url-handler-mode))
                   (unwind-protect
                       (progn
                         (url-handler-mode)
                         (find-file-noselect path-or-url))
    ...
    (if (eq buffer-checksum checksum)
        (eval-buffer buffer)
      (kill-buffer buffer) ; in the simple version kill so it is not accidentally evaled
...
(defun reval--check-buffer ()
  (let* ((url-pair (assoc :url (package-desc-extras (package-buffer-info))))
         (url (if url-pair (cadr url-pair) nil)))
    (if url
        )
      (warn "No :url section was found for this package!")

(require 'lisp-mnt)

(defun reval-is-version-of (&optional file)
...
(defun reval-get-immutable (&optional file)
  "Return the Reval-Get-Immutable: header for File or current buffer.
The value of this header should name the function in the current file
that should be used to obtain an immutable name that points to the
current remote version of the the current file."
  ;; there isn't really a datacite relation we can use here
  ;; because we would have to resolve the object of is-version-of
  ;; Has-Redirect-To-Current-Immutable ?
  ;; reval-current-immutable-version ... urg need better name for must-be-resolved
  ;; actually, no, there will always have to get a function because
  ;; we are in a situation where a remote can change so we have to have
  ;; a rule for how to get the new immutable name so always a function
  ;; or Reval-Get-Immutable: package--reval-update-imm
  (lm-with-file file
...
#+begin_src elisp
;; Latest-Version: https://raw.githubusercontent.com/tgbugs/orgstrap/master/ow-min.el
;; Dereference: https://api.github.com/repos/tgbugs/orgstrap/git/refs/heads/master#/object/sha -> master
;; Reval-Get-Imutable:
;; Reval-Update: ow-reval-update-get-immutable-url
...
#+begin_src elisp ./ow.el
;;; ow.el --- Common functionality for Orgware files. -*- lexical-binding: t -*-

;; Author: Tom Gillespie
;; URL: https://raw.githubusercontent.com/tgbugs/orgstrap/master/ow.el
;; Homepage: https://github.com/tgbugs/orgstrap
;; Version: 9999
...
(org-link-set-parameters "RUNPREV" :export #'orgstrap---nox :follow
                         (lambda (&optional nothing)
