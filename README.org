# -*- org-adapt-indentation: nil; org-edit-src-content-indentation: 0; orgstrap-cypher: sha256; orgstrap-block-checksum: fd36e5d581f48655fcdc33dd11b97dff80df47a1dbc3299230c06264723229ad; orgstrap-norm-func-name: orgstrap-norm-func--prp-1\.1; -*-
# [[orgstrap][jump to orgstrap block for this file]]
#+title: Orgstrap: Executable Org files
#+startup: showall
#+options: num:nil \n:nil

#+todo: TODO STARTED | EXPIRED DONE
...
**** dprp-1.0
A normalization function that removes docstrings.

# TODO consider whether to try to normalize lambda in these cases
# or better, to identify forms where random strings or other literals
# appearing in the body don't change the semantics, it might be more
# efficient to implement the generalized form ...

Additional forms with docstrings that we aren't fiddling with right now.
- ~lambda~
- ~cl-defun~
- ~cl-defmacro~
- ~cl-defgeneric~
- ~cl-defmethod~
A more uniform way to deal with all these cases is desired.

A better way using ~doc-string-elt~.
#+begin_src elisp
(cl-loop
 for s in '(lambda defun defun-local defmacro cl-defun
             cl-defgeneric ; missing in 24
             cl-defmacro defvar defvar-local defconst defcustom defgroup)
 collect
 (get s 'doc-string-elt))
#+end_src

#+begin_src elisp
(let* ((test '(defvar a t "b"))
       (e (get (car test) 'doc-string-elt)))
  (setcdr (nthcdr (1- e) test) (nthcdr (1+ e) test))
  test)
#+end_src

#+name: orgstrap-code-normalization--dedoc-prin1-read-progn-1.0
#+begin_src elisp :eval never
(let ((bp (read (concat "(progn\n" body "\n)")))
      print-quoted print-length print-level)
  (cl-labels
      ((dd
        (b)
        (cl-loop
         for expr in b when (listp expr) do
         (let ((c (car expr)))
           (or
            (and
             (symbolp c)
             (let* ((e (or (get c 'doc-string-elt)
                           (and (memq c '(defun-local cl-defgeneric)) 3)))
                    (d (get c 'lisp-define-type))
                    (n (and e (nthcdr (1+ e) expr))))
               (and
                e ; is a form with docstrings
                (stringp (nth e expr)) ; has a docstring
                (and
                 (or d n) ; var or doc not last
                 (dd n) ; recurse for nested
                 (or (setcdr (nthcdr (1- e) expr) n) t)))))
...
;; Version: 1.2.8                             (ref:orgstrap.el-version)
...
- Update ~orgstrap-init~ to put the Bootstrap section at the end of
  the file and to put the local variables in an archived heading
  inside of that. This pattern has been found to be quite effective
  for a number of different use cases.

- Add support for batch execution.
  - The preferred method is to use an org shebang block (see [[./shebang.org]])
  - It is also possible to maintain an automatically updating list of
    developer checksums, however this was deemed to be silly, though
    the functionality is retained. As part of these changes the code
    for generating elvs was decoupled and can now be reused.
