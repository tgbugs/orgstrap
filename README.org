...
then the =source code block= that starts closes to the beginning of the file
...
:END:
** orgstrap-mode-global
If orgstrap is installed, then having a global-orgstrap-mode which could add itself to
...
variables when global-orgstrap-mode is detect to be on might be possible, but without
