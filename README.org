...
computational artifacts, dependent only on Emacs, and hopefully in
the future any other implementation of the =org-babel= protocols.
...
be used to bootstrap an Emacs onto on a variety of systems.
...
=doom-emacs= =bin/doom= provides some inspiration here (not
that anyone should ever want to be inspired to create such
a monstrosity).

# Unfortunately I only worked out the cross-platform issues for
# the steps that came after Emacs is present on the system.

# Amazing <https://stackoverflow.com/questions/39421131/is-it-possible-to-write-one-script-that-runs-in-bash-shell-and-powershell>
# that this works.
...
We don't use hyphen-minus =-= in function names so that we can support bash
running in posix mode.

#+name: &var-cmds
: emerge apt yum dnf pacaman nix-env guix brew

#+name: the-whole-script-to-bootstrap-emacs
...
FIXME I do NOT like the fact that choco is pulling a =curl | bash= here.
...
Right now this only works with bash, zsh, and powershell because 
https://unix.stackexchange.com/a/71137 is more than I want
to try to deal with here.
...
At this point the basics of the script below should be working on
...
#+begin_src emacs-lisp :noweb no-export :tangle ./org-what-now.el.sh.ps1
":"; # -*-emacs-lisp-*-
":"; #px <<the-whole-script-to-bootstrap-emacs>>
":"; function posix () {
...
for bootsrapping. On all my windows systems =test= is available, but I suspect
that it is coming from something I have installed and cannot be relied upon.

#+name: bootstrap-from-shell-powershell
#+begin_src powershell :noweb no-export :results output
