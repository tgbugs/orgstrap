# -*- orgstrap-cypher: sha256; orgstrap-block-checksum: d1421a5bbc8df225ad4abdf9e910019126eeb6f03e405f44c6a9e8834cf5d2ab; no-klobber-test: "I should still be here after it is all over!"; -*-
...
patched to expand the body of the block prior to evaluation. The full local variables
described below have a portable version that works with unpatched versions of org-mode.
...
(defun orgstrap-init ()
  ...
  (orgstrap-add-block-checksum)
  (orgstrap--add-link-to-orgstrap-block) ; FIXME at the moment this breaks the headline, probably due to my recent switch to use evil respect visual line mode which means that others will encounter the issue as well
  (orgstrap--add-file-local-variables) ; FIXME sometimes this doesn't populate due to an out of range error
)
