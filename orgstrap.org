# -*- orgstrap-cypher: sha256; orgstrap-block-checksum: 916b06eecab63299a6396f7071d23aa9507b393440c36e6fb545ccd65484ac94; no-klobber-test: "I should still be here after it is all over!"; -*-
...
checksum of the orgstrap block has changed and if so tangle the file."
  (let* ((elv (orgstrap--read-current-local-variables))
    ...
    (unless (equal checksum-existing checksum)
...
: 6801e53195d7afe19317f0b8034ddc16f072ea334148614701d2ce2325b587b8
...
(defun orgstrap-get-block-checksum (&optional cypher)
  ...
  (orgstrap--with-block orgstrap-orgstrap-block-name
    (let ((cypher (or cypher (orgstrap--current-buffer-cypher)))
          (body-normalized (with-output-to-string (pp (read (concat "(progn\n" body "\n)"))))))
              ...
              (not (and (member lang '("elisp" "emacs-lisp"))
                        (let* ((body (orgstrap--expand-body (org-babel-get-src-block-info)))
                               (content-checksum
                                (secure-hash
                                 orgstrap-cypher
                                 (pp (read (concat "(progn\n" body "\n)"))))))
                          (message "%s %s" orgstrap-block-checksum content-checksum)
                          (equal orgstrap-block-checksum content-checksum)))))
...
=(with-output-to-string (pp (read (concat "(progn\n" body "\n)"))))= is used to normalize the code in
