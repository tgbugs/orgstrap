# -*- orgstrap-cypher: sha256; orgstrap-block-checksum: 1b4859eb92164f5ae115af796d46b68eedf843619f0f76fd7f9be4e8d2f333a0; no-klobber-test: "I should still be here after it is all over!"; -*-
...
* TODO melpa
* DONE finish patch
* TODO ruby org so that github can render footnotes correctly
...
# End:
#+end_src
*NOTE* This set of local variables will only work if you don't use noweb OR
if the version of org-mode you are using has had =org-babel-check-confirm-evaluate=
patched to expand the body of the block prior to evaluation. Version 1.0 of the
local variables described below has a portable version that works with unpatched
...
in a comment on the second line of the file."
  ...
  (let ((link-message (or link-message orgstrap-link-message)))
    (unless (looking-at-p (format "^# \\[\\[file:::%s\\]\\[.+\\]\\]$" orgstrap-orgstrap-block-name))
      (insert (format "\n# [[file:::%s][%s]]\n"
...
Use a prefix argument (i.e. C-u) to add file local variables comments instead of in a :noexport:"
  ...
  (let* ((elv (orgstrap--read-current-local-variables))
    ...
    (cond ((equal commands-existing commands) nil)
          ((not commands-existing) (mapcar (lambda (sexp) (add-file-local-variable 'eval sexp)) commands))
...
(setq-local orgstrap-local-variables-block-version 1.0)
...
Local Variables:
eval: (defun orgstrap--confirm-eval (lang body) (and (member lang '("elisp" "emacs-lisp")) (string= (secure-hash orgstrap-cypher (prin1-to-string (read (concat "(progn\n" body "\n)")))) orgstrap-block-checksum)))
eval: (setq-local org-confirm-babel-evaluate #'orgstrap--confirm-eval)
eval: (save-excursion (org-babel-goto-named-src-block "orgstrap") (org-babel-execute-src-block))
eval: (setq-local org-confirm-babel-evaluate t)
eval: (fmakunbound #'orgstrap--confirm-eval)
...
eval: (defun orgstrap--confirm-eval (lang body)
  "non-nil => prompt"
  ;;(message lang)
  ;;(message "%s" (prin1-to-string (read (concat "(progn\n" body "\n)"))))
  (not (and (member lang '("elisp" "emacs-lisp"))
       (let* ((body (orgstrap--expand-body (org-babel-get-src-block-info)))
              (content-checksum
                  (intern
                    (secure-hash
                      ...
                      (prin1-to-string (read (concat "(progn\n" body "\n)")))))))
              ;;(message "%s %s" orgstrap-block-checksum content-checksum)
              ;;(message "should we prompt? %s" (not (eq orgstrap-block-checksum content-checksum)))
              (eq orgstrap-block-checksum content-checksum)))))
eval: (setq-local org-confirm-babel-evaluate #'orgstrap--confirm-eval)
eval: (save-excursion (org-babel-goto-named-src-block "orgstrap") (org-babel-execute-src-block))
eval: (setq-local org-confirm-babel-evaluate t)
