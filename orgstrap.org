# -*- no-klobber-test: "I should still be here after it is all over!"; org-adapt-indentation: nil; org-edit-src-content-indentation: 0; orgstrap-auto-update: t; orgstrap-cypher: sha256; orgstrap-block-checksum: 07c0388b14dad296dd768420898d9e192c3148b2be58f5b43166a55518a941e3; -*-
...
keep the contents of the example blocks in sync."
  (let ((pairs `(("local-variables-example-header" ,(orgstrap--local-variables-header-string))
                 ("local-variables-example-footer" ,(orgstrap--file-local-variables-string)))))
    (mapcar (lambda (name-content) (apply #'orgstrap-update-src-block name-content)) pairs)))

(defun orgstrap--local-variables-header-string ()
...
(orgstrap--local-variables-header-string)
#+end_src

#+name: test-block
#+begin_src elisp
(24396 33321 547135 134000)
#+end_src
...
large to be able to manage. First, all uses of =(setq-local var "value")=
have to be changed to =(set (make-local-variable 'var) "value")= so that
the local variable eval code can run. However once that is done, you discover
that all of the org-babel functions are missing. Therefore, we don't support
...
By default once you run =orgstrap-init= the =orgstrap-block-checksum=
should update automatically.

This is achieved by having =orgstrap-init= install =orgstrap-auto-update=
as a file local variable set to =t= so that while you are authoring an
orgstrapped file the =orgstrap-block-checksum= update automatically whenever
a change is detected.

When publishing a file it is suggested to either set =orgstrap-auto-update= to
=nil= or remove it from the file local variables entirely. Both options have the
same effect and prevent automatic updates to the checksum. In theory this is
only an issue if you bundle the machinery for editing orgstrap files along with
the file (as is the case for this file).

If =orgstrap-auto-update= is customized globally to be =nil= then
it will not be included as a file local variable when =orgstrap-init= is called.

If you do not use =orgstrap-auto-update= then the easiest way to add the orgstrap
...
(defcustom orgstrap-auto-update t
  "A buffer local variable that controls whether or not `orgstrap-update-on-change' does anything.
If non-nil `orgstrap-update-on-change' will run, otherwise if it is nil then `orgstrap-update-on-change'
will not run AND `orgstrap-auto-update' will not be included as a file local variable by `orgstrap-init'."
  :type 'boolean
  :group 'orgstrap)

(defcustom orgstrap-on-change-hook nil
  "Hook that is run via `before-save-hook' but only runs when `orgstrap-auto-update' is
non-nil and the contents of the orgstrap block have changed."
  :type 'hook
  :group 'orgstrap)
...
(defun orgstrap-update-on-change ()
  "Use with the `before-save-hook' local variable to test whether the checksum of
the orgstrap block has changed and if so update the `orgstrap-block-checksum'
local variable and then run `orgstrap-on-change-hook'."
  (message "RUNNING %s" orgstrap-auto-update)
  (when (and (boundp 'orgstrap-auto-update) orgstrap-auto-update)
...
;; being bound to right here, though that binding could change in the future.
(add-hook 'before-save-hook #'orgstrap-update-on-change 0 t)
#+end_src
...
(defun orgstrap-init ()
  ...
  (save-excursion
    ...
    (when orgstrap-auto-update
      (add-file-local-variable-prop-line 'orgstrap-auto-update orgstrap-auto-update)
      (add-hook 'before-save-hook #'orgstrap-update-on-change 0 t))
