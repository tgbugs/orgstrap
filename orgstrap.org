# -*- no-klobber-test: "I should still be here after it is all over!"; org-adapt-indentation: nil; org-edit-src-content-indentation: 0; orgstrap-auto-update: t; orgstrap-cypher: sha256; orgstrap-block-checksum: 0e2aa349b432bb463cd9cdd283a2918e9ff3dcffda9ec7954b946f8ff4d84232; -*-
...
(defun orgstrap-update-src-block (name content-function)
  "set the content of source block with name name to content"
  (let ((content-function (if (symbol-function content-function)
                              content-function
                            (lambda () (insert content-function)))))
    (save-excursion
...
(orgstrap-update-example "test-block" (format "%s" (current-time)))

(defun orgstrap-update-examples ()
  "Use with the `before-save-hook' local variable to automatically
keep the contents of the example blocks in sync."
  ("local-variables-example-header" "TODO")  ;; do we read the local variables or do we copy the top line?
  ("local-variables-example-footer" #'orgstrap--add-file-local-variables)
  (orgstrap-update-src-block name content))
