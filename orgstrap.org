# -*- no-klobber-test: "I should still be here after it is all over!"; org-adapt-indentation: nil; org-edit-src-content-indentation: 0; orgstrap-auto-update: t; orgstrap-cypher: sha256; orgstrap-block-checksum: 05d1f57b94eb7133b25b0a7348b0ebced81592de915c11ad4712565c9f064330; -*-
...
;; lol of course it existed `org-babel-update-block-body'
(defun orgstrap-update-src-block (name content-or-thunk)
  "set the content of source block with name name to content
XXX NOTE THAT THIS CANNOT BE USED WITH EXAMPLE BLOCKS."
  (let ((content-function (if (and (symbolp content-or-thunk)
                                   (symbol-function content-or-thunk))
                              content-or-thunk
                            (lambda () (insert content-or-thunk))))
        (block (org-babel-find-named-block name)))
    (message "usb: %s %s" name content-function)
    (if block
        (save-excursion
          (org-babel-goto-named-src-block name)  ; FIXME why is not not erroring on failure !?
          (org-edit-special)
          (erase-buffer)
...
(orgstrap-update-src-block "test-block" (format "%s" (current-time)))
;;(orgstrap-update-src-block "local-variables-example-header" (format "%s" (current-time)))
...
keep the contents of the example blocks in sync."
  (let ((pairs '(("local-variables-example-header" "TODO")
                 ;; do we read the local variables or do we copy the top line?
                 ("local-variables-example-footer" #'orgstrap--add-file-local-variables))))
...
(defcustom orgstrap-auto-update t
  "A buffer local variable that controls whether or not `orgstrap-block-checksum' is
updated automatically or not when the file is saved. When not set or non-nil defaults
to the value customized here if orgstrap is installed, otherwise defaults to t"
  :type 'boolean
  :group 'orgstrap)
...
checksum of the orgstrap block has changed and if so tangle the file."
  (when (or (not (boundp 'orgstrap-auto-update)) orgstrap-auto-update)
